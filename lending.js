...
export const LENDING_DOCS = [
  ...
  },
  {
    ...
    text: `The stack this prototype is aimed at is the one Symple Lending's Customer Engagement team publishes in its conversational AI engineering role: Node and TypeScript on AWS Lambda behind API Gateway, deployed with CDK, with DynamoDB and S3 for state and artifacts, SQS, SNS and EventBridge for orchestration, Twilio for telephony, a voice agent platform such as ElevenLabs or a virtual agent product for speech, HubSpot as the CRM of record, Redshift or Athena for analytics, and CloudWatch with structured logging for observability. The controls in this corpus are deliberately transport-agnostic: guardrail evaluation, redaction and disclosure records sit between the conversation and the model, so they hold identically on a web chat, an inbound Twilio call and an outbound campaign, and they survive swapping the speech or model vendor. That portability is the argument for putting the policy at the protocol layer rather than in a system prompt, which is what Protocol Layer Prompt Engineering means in practice [webproof].`
  },
  {
    id: "prototype-brief",
    title: "Prototype brief: K8T for consumer lending",
    kind: "concept",
    url: "/api/corpus/prototype-brief",
    tags: ["Symple", "Symple Lending", "CDO", "prototype", "brief", "why", "who built this", "purpose"],
    text: `K8T is an independent working prototype built by Lawrence John Reilly Jr., aimed at the conversational AI work Symple Lending runs out of its Customer Engagement team. It is not affiliated with, commissioned by or endorsed by Symple Lending, and the policy text in this corpus is illustrative rather than approved compliance language. The premise is narrow: a conversational agent in consumer lending is straightforward to build and hard to govern, so the prototype demonstrates the governance half. Borrower mode runs a deterministic guardrail layer that redacts nonpublic personal information before the model call, blocks quoted rates and approval promises outright, escalates prohibited-basis and complaint paths to a human, and attests every turn against a versioned policy digest. The suite persona explains the protocol work the controls are drawn from. Both personas run on the same engine, on a public URL, so the claims are checkable rather than described in a deck.`
  },
  {
    id: "control-mapping",
