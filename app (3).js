...
// Suggestions are sampled at random on every refresh, and never repeat the set
// that was just on screen. Nothing here is a fixed script.
const lastShown = { suite: [], borrower: [] };

function sample(pool, exclude, n) {
  const candidates = pool.filter((q) => !exclude.includes(q));
  const bag = (candidates.length >= n ? candidates : pool).slice();
...
function renderChips() {
  const picks = sample(POOLS[persona], lastShown[persona], BATCH);
  lastShown[persona] = picks;
...
// Brings the suggestions back after a turn, and reshuffles them if they are
// already showing.
$("refreshChips").addEventListener("click", renderChips);
